...
> **设计理念**：三层物理结构 · 去上下文化 I/O · 自愈兜底 · Token 效率 · 四维创意引擎 · 硬约束自动修复 · 开题直出
...
本项目是 **Skill 集合项目**，而非传统应用代码库。项目内除 `core/scripts/` 下 4 个可执行 Python 脚本（供各平台 Skill 按需调用）与 `tests/` 自测脚本外，主体产物均为面向各 AI 平台运行时的指令文本（Markdown / YAML frontmatter / JSON 配置），由底层大模型运行时加载并解释执行。
...
ThesisMinerSkill/
├── core/                           # 共享资源层（平台无关）
│   ├── scripts/                    # 可执行脚本（4个Python文件）
│   │   ├── lineage_parser.py
│   │   ├── idea_generator.py
│   │   ├── constraint_checker.py
│   │   └── report_generator.py
│   ├── references/                 # 静态参考数据（4个文件）
│   │   ├── constraints.json
...
V2.1 将每个平台 Skill 从单文件结构重构为**三层物理结构**，并配套去上下文化 I/O、自愈兜底与 Token 效率优化，解决旧版"上下文成本高、无法渐进式披露、I/O 无结构化校验、缺乏自愈逻辑"四大痛点。

### 3.1 三层物理结构

每个平台 Skill 采用三层物理结构，职责分明、按需加载：
...
ThesisArchitect 的核心业务流程为无状态六步工作流，每步对应 `core/scripts/` 下一个可执行脚本，所有平台产物均按此顺序执行：

1. **I/O 意图识别**：解析用户指令，确定输入源（文件/对话/混合）与输出目标（文件/对话），按 `input_schema.json` 校验。若关键信息（学位、导师项目或同门基础）缺失，先以简短提问补齐，不凭空假设。
2. **上下文加载与谱系解析**：调用 `lineage_parser.py` 的 `parse_lineage(text)`，构建内部 `LineageGraph`。执行实体抽取（导师项目名称/目标/技术路线、同门论文标题/方法/局限性）与边缘探测（标记"未来工作""受限于算力/数据未展开"等高价值切入点）。
3. **四维创意发散**：调用 `idea_generator.py` 的 `generate_ideas(lineage_graph, strategy, degree)`，并行执行四个策略生成原始候选，按 `scoring_weights.json` 权重自评分，丢弃 < 6 分的方向，保留 Top 3–5 个方向。
...
5. **硬约束拦截与修复**：调用 `constraint_checker.py` 的 `check_and_repair(proposal)`，依据 `constraints.json` 执行标题格式、学术日历、文献基线、逻辑自洽四类校验，对不达标项执行确定性自动修复，无法修复的标记 `WARNING`。
6. **多态输出**：调用 `report_generator.py` 的 `generate_report(proposal)` 渲染结果，按 `output_schema.json` 标准化输出。若用户要求开题报告，提取最优提案按 `report_template.md` 五大模块直出完整 Markdown；若只要候选提案，按字段结构化输出 Top 3–5；若要求文件输出，序列化为 JSON 与 Markdown 写入 `output/` 目录并回复路径摘要。
...
| `core/scripts/idea_generator.py` | `generate_ideas(lineage_graph, strategy, degree)` | `LineageGraph` + 策略（advisor_extension/peer_inheritance/cross_domain/contradiction_driven/all）+ 学位（master/phd） | 候选提案列表（含 `title`/`score` 等字段，已过滤 < 6 分） |
| `core/scripts/constraint_checker.py` | `check_and_repair(proposal)` | 单个提案 dict | 修复后提案 + `WARNING` 标记（标题/学术日历/文献基线/逻辑自洽） |
| `core/scripts/report_generator.py` | `generate_report(proposal)` | 最优提案 dict | 开题报告 Markdown（五大模块）+ 文件输出路径（文件模式下） |
...
`tests/` 目录对 `core/scripts/` 下 4 个脚本与 `core/schema/` 下 2 个 Schema 进行自测，共 **28 个测试用例**，覆盖核心计算路径与边界场景。
...
| `tests/test_idea_generator.py` | `generate_ideas()`：四策略各 1 例 + 自评分过滤 + all 策略 | 6 |
| `tests/test_constraint_checker.py` | `check_and_repair()`：标题超长/动词前置/套路模式、学术日历、文献基线、逻辑自洽 | 6 |
| `tests/test_report_generator.py` | `generate_report()`：五大模块、模板填充、文件输出、标准输出 | 4 |
| `tests/test_schema.py` | `input_schema.json` / `output_schema.json`：合法性、必填字段、枚举值、status 枚举、样例数据校验 | 7 |
...
预期输出：28 个用例全部通过（`28 passed`）。测试不依赖第三方包以外的网络与外部服务，脚本沙盒写入仅落在临时 `output/` 目录。
...
| 自测失败 | `core/` 脚本或 Schema 被改动后未同步更新测试 | 在项目根目录运行 `python -m pytest tests/ -v`，定位失败用例；修复脚本/Schema 后重新运行，确保 28 个用例全部通过 |
